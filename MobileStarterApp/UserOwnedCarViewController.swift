...
class UserOwnedCarViewController: UIViewController, CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        if (self.startedDriving) {
            ...
            let mphSpeed = round((locations.last?.speed)! * 60 * 60 / 16.0934)/100
            self.titleLabel.text = "Speed: \(max(0, mphSpeed)) MPH"
            let center = CLLocationCoordinate2DMake((locations.last?.coordinate.latitude)!, (locations.last?.coordinate.longitude)!)
    ...
    func renderMapMatchedLocation() {
        ...
        API.doRequest(request) { (response, jsonArray) -> Void in
            stats = Path.fromDictionary(array: jsonArray as NSArray)
            if stats.count > 0 {
                if let stat: Path = stats[0] {
                    DispatchQueue.main.async(execute: {
                        for coordinate in stat.coordinates! {
                            let circle = MKCircle(center: CLLocationCoordinate2DMake((coordinate[1] as AnyObject).doubleValue!, (coordinate[0] as AnyObject).doubleValue!), radius: 10);
    ...
    func useExistingReservation() {
        ...
        API.doRequest(request) { (response, jsonArray) -> Void in
            DispatchQueue.main.async(execute: {
                self.reservations = ReservationsData.fromDictionary(jsonArray as NSArray)
