...
class ViewController: UIViewController, CLLocationManagerDelegate, UIViewControllerTransitioningDelegate {
            ...
            if (ViewController.mqtt == nil && ViewController.needCredentials) {
                ...
                API.doRequest(request) { (httpResponse, jsonArray) -> Void in
                    ...
                    DispatchQueue.main.sync(execute: {
                        ...
                        let clientIdPid = "d:\((deviceCredentials.object(forKey: "org"))!):\((deviceCredentials.object(forKey: "deviceType"))!):\((deviceCredentials.object(forKey: "deviceId"))!)"
                        ViewController.mqtt = CocoaMQTT(clientID: clientIdPid, host: "\((deviceCredentials.object(forKey: "org"))!).messaging.internetofthings.ibmcloud.com", port: 8883)
                        
                        if let mqtt = ViewController.mqtt {
                            mqtt.username = "use-token-auth"
                            mqtt.password = deviceCredentials.object(forKey: "token") as? String
                            mqtt.keepAlive = 90
                            mqtt.delegate = self
                            mqtt.enableSSL = true
                        }
    ...
    func sendLocation(_ userLocation: CLLocation) {
        ...
        var data: [String: AnyObject] = [
            "speed": max(0, Int(userLocation.speed * 60 * 60 / 1000)) as AnyObject,
            "lng": userLocation.coordinate.longitude as AnyObject,
            "lat": userLocation.coordinate.latitude as AnyObject,
            "ts": dateFormatter.string(from: Date()) as AnyObject,
            "id": ViewController.mobileAppDeviceId as AnyObject,
            "status":  (ViewController.tripID != nil ? "Unlocked" : "Locked") as AnyObject
        ]
        if(ViewController.tripID != nil){
...
extension ViewController: CocoaMQTTDelegate {
    ...
    func mqtt(_ mqtt: CocoaMQTT, didConnectAck ack: CocoaMQTTConnAck) {
        print("connected")
        sendLocation(locationManager.location!) // initial location
    ...
    func mqtt(_ mqtt: CocoaMQTT, didReceiveMessage message: CocoaMQTTMessage, id: UInt16 ) {
        print("didReceiveMessage: \(message.string) with id \(id)")
    }
