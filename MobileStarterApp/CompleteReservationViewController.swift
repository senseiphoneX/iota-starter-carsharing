...
class CompleteReservationViewController: UIViewController {
    ...
    @IBAction func unlockCarAction(_ sender: AnyObject) {
        ...
        API.doRequest(request) { (httpResponse, jsonArray) -> Void in
            let reservations = ReservationsData.fromDictionary(jsonArray as NSArray)
            ...
            case 200:
                DispatchQueue.main.async(execute: {
                    if command == "lock" {
                        self.unlockButton.setTitle("Unlock the car", for: UIControlState.normal)
                        self.unlockMessageLabel.text = ""
                    } else {
                        self.unlockButton.setTitle("Lock the car", for: UIControlState.normal)
