...
class ProfileViewController: UIViewController, MessageViewController {
    ...
    func getDriverStats() {
        ...
        API.doRequest(request) { (response, jsonArray) -> Void in
            stats = DriverStatistics.fromDictionary(jsonArray as NSArray)
            if stats.count > 0 {
                if let stat: DriverStatistics = stats[0] {
                    DispatchQueue.main.async(execute: {
                        ...
                        self.timesOfDaySortedKeys = (self.timesOfDay! as NSDictionary).keysSortedByValue(comparator: {
                        ...
                        self.roadTypesSortedKeys = (self.roadTypes! as NSDictionary).keysSortedByValue(comparator: {
                                ($1 as! NSNumber).compare($0 as! NSNumber)
                            }) as [AnyObject]
                        
                        self.trafficConditions = self.stat?.speedPattern?.toDictionary()
                        self.trafficConditions?.setValue(nil, forKey: "totalDistance")
                        self.trafficConditionsSortedKeys = (self.trafficConditions! as NSDictionary).keysSortedByValue(comparator: {
                                ($1 as! NSNumber).compare($0 as! NSNumber)
                            }) as [AnyObject]
