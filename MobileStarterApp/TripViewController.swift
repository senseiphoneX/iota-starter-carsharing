...
class TripViewController: UIViewController {
    ...
    func getDriverBehavior() {
        ...
        var stats:[Trip] = []
        API.doRequest(request) { (response, jsonArray) -> Void in
            stats = Trip.fromDictionary(jsonArray as NSArray)
            if stats.count > 0 {
                ...
                if let stat: Path = stats[0] {
                    DispatchQueue.main.async(execute: {
                        ...
                        for coordinate in stat.coordinates!{
                            points.append(CLLocationCoordinate2DMake((coordinate[1] as AnyObject).doubleValue!, (coordinate[0] as AnyObject).doubleValue!))
                        }
                        let polyline = MKPolyline(coordinates: UnsafeMutablePointer(mutating: points), count: points.count)
